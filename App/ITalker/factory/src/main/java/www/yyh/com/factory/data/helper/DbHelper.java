...
public class DbHelper{
     ...
     * 观察者的集合
     * Class<?> ：观察的表
     * Set<ChangedListener>：每一个表对应的观察者有很多，listener不变所以用set
     */
    private final Map<Class<?>,Set<ChangedListener>> chanchedListeners =new HashMap<>();
    ...
    private  <Model extends BaseModel> Set<ChangedListener> getListeners(Class<Model> modelClass){
        if (chanchedListeners.containsKey(modelClass)){
            return chanchedListeners.get(modelClass);
        }
        return null;
    }
    ...
    public static <Model extends BaseModel> void  addChangedListener(final Class<Model> tClass,
        ...
        if (changedListeners==null){
            ...
            //添加到总的Map
            instance.chanchedListeners.put(tClass,changedListeners);
        }
     ...
     * @param <Model> 这个实例的泛型，限定条件是BaseModel
     */
    private final <Model extends BaseModel> void notifySave(
            final Class<Model> tClass, final Model... models){
        // 找监听器
     ...
     * @param <Model> 这个实例的泛型，限定条件是BaseModel
     */
    private final <Model extends BaseModel> void notifyDelete(
            final Class<Model> tClass, final Model... models){
        // 找监听器
        final Set<ChangedListener> listeners = getListeners(tClass);
        if (listeners!=null&&listeners.size()>0){
